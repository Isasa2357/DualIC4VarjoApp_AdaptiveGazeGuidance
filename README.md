...
IC4 SDKは自動取得されません。インストーラが設定する`IC4PATH`を利用するか、CMakeへ`IC4_SDK_ROOT`を渡してください。Varjo Native SDKも自動取得されないため、`VARJOXR_VARJO_SDK_ROOT`を指定します。
...
### IC Capture 4のJSONを左右別に使用

```bat
out\build\default\Release\DualIC4VarjoApp.exe ^
  --left-device-index 0 ^
  --right-device-index 1 ^
  --left-json C:\config\left.json ^
  --right-json C:\config\right.json ^
...
CSVはUTF-8 BOM付きです。サンプルCSVと同様にVarjoへ提出したレンダーフレームごとに1行を書きます。
