...
IC4Extで2台のカメラをD3D12テクスチャとして取得し、`D3D12FrameSyncThread`で同期した左右フレームを、VarjoXRの1枚のPlaneへ左右眼別テクスチャとして表示する実験用アプリケーションです。Varjoへ提出した各レンダーフレームと、その時点で表示していた左右カメラフレーム、Plane位置・サイズ、同期状態をCSVへ記録します。
...
操作は押下エッジで処理します。キーを押しっぱなしにしても連続変更せず、1回押すごとに0.01 mずつ変更します。
...
リサイズではPlaneの縦横比を維持し、幅と高さを同じ比率で変更します。最小幅は0.05 mです。位置またはサイズを変更すると、現在の`x / y / z / width / height`をコンソールへ表示します。

Planeサイズは最初の同期フレームを受け取った時点で一度だけ初期化します。その後、新しいカメラフレームが届いてもキー操作で変更したサイズは保持されます。初期位置と初期サイズは`--plane-x-m`、`--plane-y-m`、`--plane-distance-m`、`--plane-width-m`、`--plane-height-m`で指定できます。
...
- レンダー行番号、提出時刻、提出成功状態
- 新しい同期フレームへ切り替えたか
- Planeをそのフレームで移動・リサイズしたか
- Planeの配置方式、`x / y / z`、幅、高さ
