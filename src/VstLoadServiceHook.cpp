...
void ResolveOutputConfiguration(int argc, char** argv)
{
    if (const auto active = ActiveExperimentOutputLayout()) {
        gOutputDirectory = active->directory;
        gBaseFilename = Utf8ToWide(active->resolvedProjectName);
        return;
    }
    ...
    gOutputDirectory =
        baseDirectory / "service_load" / (project + "_vst");
...
void StartService(std::shared_ptr<varjo_Session> session) noexcept
{
    try {
        ...
        std::cout
            ...
            << "[VST] output directory: " << gOutputDirectory.string() << '\n'
...
void VstLoadServiceHook::configure(int argc, char** argv)
{
    stop();
    std::lock_guard<std::mutex> lock(gStateMutex);
    ResolveOutputConfiguration(argc, argv);
    gLastError.clear();
...
{
    if (gServiceRaw.load(std::memory_order_acquire)) return;
...
void VstLoadServiceHook::stop() noexcept
{
    ...
    if (service) {
        ...
        gFinalLeftReceived.store(service->leftReceivedFrameCount(), std::memory_order_release);
        gFinalRightReceived.store(service->rightReceivedFrameCount(), std::memory_order_release);
        gFinalLeftProcessed.store(service->leftProcessedFrameCount(), std::memory_order_release);
        gFinalRightProcessed.store(service->rightProcessedFrameCount(), std::memory_order_release);
        gFinalDropped.store(service->droppedFrameCount(), std::memory_order_release);
        gFinalWriteFailures.store(service->writeFailureCount(), std::memory_order_release);
    }
}
...
{
    VarjoVSTService* service = gServiceRaw.load(std::memory_order_acquire);
    return service ? service->leftReceivedFrameCount() : gFinalLeftReceived.load(std::memory_order_acquire);
}

std::uint64_t VstLoadServiceHook::rightReceivedCount() noexcept
{
    VarjoVSTService* service = gServiceRaw.load(std::memory_order_acquire);
    return service ? service->rightReceivedFrameCount() : gFinalRightReceived.load(std::memory_order_acquire);
}

std::uint64_t VstLoadServiceHook::leftProcessedCount() noexcept
{
    VarjoVSTService* service = gServiceRaw.load(std::memory_order_acquire);
    return service ? service->leftProcessedFrameCount() : gFinalLeftProcessed.load(std::memory_order_acquire);
}

std::uint64_t VstLoadServiceHook::rightProcessedCount() noexcept
{
    VarjoVSTService* service = gServiceRaw.load(std::memory_order_acquire);
    return service ? service->rightProcessedFrameCount() : gFinalRightProcessed.load(std::memory_order_acquire);
}

std::uint64_t VstLoadServiceHook::droppedCount() noexcept
{
    VarjoVSTService* service = gServiceRaw.load(std::memory_order_acquire);
    return service ? service->droppedFrameCount() : gFinalDropped.load(std::memory_order_acquire);
}

std::uint64_t VstLoadServiceHook::writeFailureCount() noexcept
{
    VarjoVSTService* service = gServiceRaw.load(std::memory_order_acquire);
    return service ? service->writeFailureCount() : gFinalWriteFailures.load(std::memory_order_acquire);
}
